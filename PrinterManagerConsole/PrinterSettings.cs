...
namespace PrinterManagerConsole
{
    class PrinterSettings
    {
        ...
        {
            sPrinterName = ParsePrinter(sPrinterName);

            GetPrinterInfo(sPrinterName);
	...
	WHERE Name = '" + sPrinterName.Replace("\\", "\\\\") + "'";
        ...
        //Get all printers
        public static void GetAllPrinters(string sPrinterName)
        {
            sPrinterName = ParsePrinter(sPrinterName);
        ...
        public static string ParsePrinter(string sPrinterName)
        {
            string[] server;

            oManagementScope = new ManagementScope(ManagementPath.DefaultPath); 

            if(sPrinterName.StartsWith("\\"))
            {
                server = sPrinterName.Split(new Char[] { '\\' });

                if (server.Length > 3)
                {
                    sPrinterName = server[3];
                }
                

                string serverPath = String.Format(@"\\{0}\root\cimv2", server[2]);
                oManagementScope = new ManagementScope(serverPath);
            }
